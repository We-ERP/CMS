...
2. امسح الكود القديم (أو خد نسخة احتياطية منه لو فيه دوال بتستخدمها في مكان تاني) والصق محتوى `google-apps-script/Code.gs`.
3. لو السكريبت مش مربوط تلقائيًا بالشيت، غيّر `SpreadsheetApp.getActiveSpreadsheet()` إلى `SpreadsheetApp.openById("SHEET_ID")`.
4. **Deploy → Manage deployments → ✏️ Edit → New version → Deploy** (عشان نفس رابط الـ `/exec` يشتغل بالكود الجديد).
